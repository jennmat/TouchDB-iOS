...
TestCase(TDPuller_DocIDs) {
    RequireTestCase(TDPusher);
    
    TD_DatabaseManager* server = [TD_DatabaseManager createEmptyAtTemporaryPath: @"TDPuller_DocIDs_Test"];
    TD_Database* db = [server databaseNamed: @"db"];
    [db open];
    ...
    NSLog(@"GOT DOCS: %@", [[db getAllDocs:nil] description]);
    
    CAssertEq(db.documentCount, 1u);
    CAssertEq(db.lastSequence, 2);
